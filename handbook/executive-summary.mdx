...
3. **The runtime is a combination.** Use an embeddable execution kernel, a horizontally scalable runtime service, a durable-workflow adapter, and a platform-owned canonical run journal.
4. **Execution truth is platform-owned.** Workflow-engine history and traces support operations, but the canonical run journal defines stable execution semantics.
...
- Relational state store and append-only platform run journal.
