...
Evaluation-driven development applies test-first discipline to probabilistic systems.
...
7. Compare distributions across `TrialRun`s where stochasticity matters.
...
A baseline pins subject versions, model-routing profile, dataset, evaluators, trial count, environment digest, and thresholds. Compare a candidate against the strongest relevant baseline, not a deliberately weak implementation.

## Stochastic confidence

Report pass rate, mean, dispersion, confidence interval where appropriate, worst critical slice, and repeated-trial failure modes. A single passing sample is weak evidence.
