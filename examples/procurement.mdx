---
title: Worked example — procurement agent
description: End-to-end durable, multi-agent, approval-bound purchase-order execution with recovery, evaluation, and billing.
---

# Worked example — procurement agent

> **Status: Worked example.**

## Scenario

Acme EU asks the production agent to create a €180,000 purchase order from contract `C-882` and quote `Q-19` with an effective per-run cost limit of USD 8.00 under a USD 12.00 deployment ceiling.

Requirements:

- Read contract and vendor data in the EU region.
- Reconcile line items with model reasoning.
- Run independent research and compliance checks.
- Require procurement-manager and finance-controller approval.
- Create exactly one order despite an injected network timeout.
- Evaluate correctness and policy compliance.
- Attribute all cost and package usage.

## Architecture trace

```mermaid
sequenceDiagram
  participant User
  participant API
  participant Runtime
  participant Journal
  participant Research
  participant Compliance
  participant Model
  participant Policy
  participant Approvers
  participant ERP
  participant Eval
  participant Meter

  User->>API: Start procurement run with USD 8 limit
  API->>Journal: run.started with signed deployment snapshot
  Runtime->>Research: Child/read-only research task
  Runtime->>Compliance: Child/policy-read task
  Research-->>Runtime: Research artifact
  Compliance-->>Runtime: Compliance artifact
  Runtime->>Model: Propose normalized order arguments
  Runtime->>Policy: Authorize €180,000 create Effect
  Policy-->>Runtime: Require dual approval
  Runtime->>Journal: approval.requested + checkpoint
  Approvers->>API: Approve exact action digest
  API->>Runtime: Resume
  Runtime->>Policy: Revalidate
  Runtime->>Journal: effect.planned + idempotency key
  Runtime->>ERP: Invocation 1 — create order
  ERP--xRuntime: Response connection fails after commit
  Runtime->>Journal: effect.outcome_unknown
  Runtime->>ERP: Invocation 2 — reconcile by idempotency key
  ERP-->>Runtime: PO-2026-8841 exists
  Runtime->>Journal: effect.reconciled + run.completed
  Journal-->>Eval: Events and artifacts
  Journal-->>Meter: Usage facts
```

## Ownership

| Operation | Owner | Evidence |
|---|---|---|
| Authenticate and route tenant | Application/identity | Auth audit and EU cell decision |
| Resolve behavior | Control plane | Deployment digest |
| Create and progress run | Runtime Service and Execution Kernel | Canonical run events |
| Compile context | Runtime | Source refs and digest |
| Delegate checks | Runtime/policy | Child run IDs and grants |
| Propose order | Model gateway | Normalized request/result and usage |
| Validate totals | Domain/task activity | Deterministic validation event |
| Approve | Approval service | Signed decisions bound to effect/action digest |
| Create and reconcile | Tool gateway/runtime | One Effect, two Invocations, provider evidence |
| Complete | Run aggregate | Terminal result and artifacts |
| Evaluate | Evaluation plane | Versioned evaluation result |
| Bill | Metering plane | Immutable usage records |

## Deterministic assertions

```text
vendor matches contract
total equals €180,000 and currency is EUR
every line item traces to contract or quote
both approvals precede first mutating Invocation
action digest equals Effect input digest
exactly one ERP order exists for the idempotency key
no cross-tenant or cross-region access occurred
effective USD 8 run budget was enforced
```

## Failure recovery

The timeout is classified `effectMayHaveOccurred=true` and `safeToRetry=false`. The runtime retains one logical create `Effect` and performs a separate reconciliation `Invocation` rather than sending another create request.

## Compensation boundary

The example demonstrates reconciliation, not rollback. If the domain permits cancelling a created order, cancellation is a separately authorized compensation `Effect` with its own invocations, audit, and failure semantics. See [Saga and compensation](/patterns/saga-and-compensation).
