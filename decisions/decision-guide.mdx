...
> **Status: Informative.** Canonical names are defined by the [ARA specification](/specification/index).
...
Use an activity for one bounded semantic unit with one input and result contract. Use a referenced workflow when internal work has dependencies, branches, waits, several activities, or separately meaningful policy or evaluation.

## Inline or child?

Use inline execution when work shares parent state, budget, cancellation, and lifecycle. Use a child `WorkflowRun` or `AgentRun` when it needs independent identity, state, budget, deadline, permissions, cancellation, failure, result, or evaluation.
...
| Prompt, context, arguments, or objective changes | New `Effect` |
...
| Large or reusable content | Artifact |
| Reusable governed knowledge | Memory record |
| Business truth | Domain aggregate or reference |
...
- Durable backend: timers, signals, suspension, and recovery behind `DurableExecutionPort`.
- Lease or actor: temporary serialization and ownership.
